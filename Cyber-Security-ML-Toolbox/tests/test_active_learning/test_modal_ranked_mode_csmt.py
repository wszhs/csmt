...
with plt.style.context('seaborn-white'):
    ...
    plt.scatter(transformed_data[training_indices, 0], transformed_data[training_indices, 1], c='k', label='labeled',s=5)
...
for index in range(N_QUERIES):
    ...
    if index <1:
        ...
        with plt.style.context('seaborn-white'):
            plt.figure(figsize=(6, 6), dpi=100)
            plt.scatter(transformed_data[:, 0], transformed_data[:, 1], c='0.8', label='unlabeled',s=5)
            plt.scatter(transformed_data[training_indices, 0], transformed_data[training_indices, 1], c='k',
                        label='training',s=5)
            plt.scatter(selected[:, 0], selected[:, 1], c='r', label='1st query',s=5)
            plt.title('Unlabeled and labeled data')
            plt.legend()
            plt.show()
