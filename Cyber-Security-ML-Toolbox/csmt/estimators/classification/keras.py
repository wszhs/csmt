...
class KerasClassifier(ClassGradientsMixin, ClassifierMixin, KerasEstimator):
    ...
    def predict(self, x: np.ndarray, batch_size: int = 128, **kwargs) -> np.ndarray:
        ...
        predictions = np.zeros((x_preprocessed.shape[0], self.nb_classes), dtype=CSMT_NUMPY_DTYPE)
    ...
    def fit(self, x: np.ndarray, y: np.ndarray, X_val,y_val,batch_size: int = 128, nb_epochs: int = 2, **kwargs) -> None:
        """
    ...
    def get_activations(
        ...
        activations = np.zeros((x_preprocessed.shape[0],) + output_shape[1:], dtype=CSMT_NUMPY_DTYPE)
    ...
    def __repr__(self):
        repr_ = (
            "%s(model=%r, use_logits=%r, clip_values=%r, input_layer=%r, output_layer=%r)"
            % (
