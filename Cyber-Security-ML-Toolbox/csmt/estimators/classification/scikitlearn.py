...
class ScikitlearnClassifier(ClassifierMixin, ScikitlearnEstimator): 
    ...
    def fit(self, x: np.ndarray, y: np.ndarray,X_val:np.ndarray,y_val:np.ndarray, **kwargs) -> None:
        """
    ...
    def predict(self, x: np.ndarray, **kwargs) -> np.ndarray:
        ...
        if hasattr(self.model, "predict_proba") and callable(getattr(self.model, "predict_proba")):
            ...
            y_pred = to_categorical(self.model.predict(x), nb_classes=self.model.classes_.shape[0],)
        else:
    ...
    def loss_gradient(self, x: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        if self.nb_classes == 2:
            for i_sample in range(num_samples):
                gradients[i_sample, :] += (
                    class_weight[1] * (1.0 - y[i_sample, 1])
                    - class_weight[0] * (1.0 - y[i_sample, 0])
...
class ScikitlearnSVC(ClassGradientsMixin, LossGradientsMixin, ScikitlearnClassifier):
    ...
    def class_gradient(self, x: np.ndarray, label: Union[int, List[int], None] = None, **kwargs) -> np.ndarray:
        ...
        if isinstance(self.model, sklearn.svm.SVC):
            ...
            if label is None:
                gradients = np.zeros((x.shape[0], self.nb_classes, x.shape[1],))
