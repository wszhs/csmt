...
class BayesEnsemble(ClassifierMixin, BaseEstimator):
    ...
    def __init__(
        ...
        self.weight=1.0/len(self.model)*np.ones(len(self.model),dtype=CSMT_NUMPY_DTYPE)
    ...
    def bayes_ensemble(self,X_test):
        ...
        y_pred_all=np.zeros((X_test.shape[0],self._nb_classes),dtype=CSMT_NUMPY_DTYPE)
        for i in range(0,len(self.model)):
