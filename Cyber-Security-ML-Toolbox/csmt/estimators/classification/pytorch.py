...
class PyTorchClassifier(ClassGradientsMixin, ClassifierMixin, PyTorchEstimator): 
    ...
    def fit(self, x: np.ndarray, y: np.ndarray,x_val: np.ndarray, y_val: np.ndarray,batch_size: int = 128, nb_epochs: int = 1, **kwargs) -> None:
        """
        ...
        :param y_val: Target values (class labels) one-hot-encoded of shape (nb_samples, nb_classes) or index labels of
                  shape (nb_samples,).
        :param batch_size: Size of batches.
        ...
        for _ in range(nb_epochs):
            ...
            # Test Accuracy
            self._model.eval()
            num_batch_val = int(np.ceil(len(x_val) / float(batch_size)))
            ind_val = np.arange(len(x_val))
            accurate_count = 0
            with trange(num_batch_val) as t:
                for m in t:
                    t.set_description('Test %i' %m)
                    i_batch = torch.from_numpy(x_val[ind_val[m * batch_size : (m + 1) * batch_size]]).to(self._device)
                    o_batch = torch.from_numpy(y_val[ind_val[m * batch_size : (m + 1) * batch_size]]).to(self._device)

                    # Perform prediction
                    outputs = self._model(i_batch)
                    out= outputs[-1].argmax(1)
                    labels=o_batch.flatten()
                    accurate_count+=(out==labels).sum()
            accuracy=accurate_count/len(y_val)
            # print(accuracy)
            # wandb.log({'Test_accracy':accuracy})
    ...
    def get_activations(
        ...
        for m in range(num_batch):
            ...
            layer_output = self._model(torch.from_numpy(x[begin:end]).to(self._device))[layer_index]
    ...
    def __setstate__(self, state: Dict[str, Any]) -> None:
        ...
        full_path = os.path.join(config.CSMT_DATA_PATH, state["model_name"])
