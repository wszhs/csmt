...
class TensorFlowClassifier(ClassGradientsMixin, ClassifierMixin, TensorFlowEstimator):  # lgtm [py/missing-call-to-init]
    ...
    def fit(self, x: np.ndarray, y: np.ndarray,X_val,y_val, batch_size: int = 8, nb_epochs: int = 10, **kwargs) -> None:
        """
        ...
        for _ in range(nb_epochs):
            ...
            # result=get_class_scores(y, y_pred)
            # print(result)   
...
class TensorFlowV2Classifier(ClassGradientsMixin, ClassifierMixin, TensorFlowV2Estimator):
    ...
    def get_activations(
        ...
        """
        import tensorflow as tf  # lgtm [py/repeated-import]
        from art.config import CSMT_NUMPY_DTYPE
