...
import numpy as np
import Markov.markov_model as MK
import time
...
class SLFeature(object):
    
    def __init__(self, lenPro=0.9,order=1):
        self.smarkovs = None
        self.lmarkovs = None
        self.lenPro = lenPro
        self.order=order
    ...
    def get_feature(self,status,length):
        self._getMarkov(status, length)
        data = [(apps, appl) for apps, appl in zip(status, length)]
        feature, label = self._getfeature(data, (self.smarkovs, self.lmarkovs))
