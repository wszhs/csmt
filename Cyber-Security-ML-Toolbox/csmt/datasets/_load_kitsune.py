...
def load_kitsune():
    ...
    # df_0 = df_0.sample(frac=0.05, random_state=20)
...
def load_kitsune_scan():

    X = np.load('csmt/datasets/data/Kitsune/OS_Scan/kitsune_feature_data.npy')
    y= np.load('csmt/datasets/data/Kitsune/OS_Scan/labels.npy')
    X=pd.DataFrame(X)
    y=pd.DataFrame(y)
    df= pd.concat([X,y],axis=1)
    # df.to_csv('csmt/datasets/data/Kitsune/mirai/mirai.csv')
