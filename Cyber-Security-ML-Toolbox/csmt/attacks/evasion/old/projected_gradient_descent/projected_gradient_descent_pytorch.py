...
class ProjectedGradientDescentPyTorch(ProjectedGradientDescentCommon):
        ...
        if mask is not None:
            ...
            if len(mask.shape) == len(x.shape):
                dataset = torch.utils.data.TensorDataset(
                    torch.from_numpy(x.astype(CSMT_NUMPY_DTYPE)),
                    torch.from_numpy(targets.astype(CSMT_NUMPY_DTYPE)),
                    torch.from_numpy(mask.astype(CSMT_NUMPY_DTYPE)),
                )
                ...
                dataset = torch.utils.data.TensorDataset(
                    torch.from_numpy(x.astype(CSMT_NUMPY_DTYPE)),
                    torch.from_numpy(targets.astype(CSMT_NUMPY_DTYPE)),
                    torch.from_numpy(np.array([mask.astype(CSMT_NUMPY_DTYPE)] * x.shape[0])),
                )

        else:
            dataset = torch.utils.data.TensorDataset(
                torch.from_numpy(x.astype(CSMT_NUMPY_DTYPE)), torch.from_numpy(targets.astype(CSMT_NUMPY_DTYPE)),
            )
    ...
    def _apply_perturbation(
        ...
        eps_step = np.array(eps_step, dtype=CSMT_NUMPY_DTYPE)
