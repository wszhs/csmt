...
class ProjectedGradientDescentTensorFlowV2(ProjectedGradientDescentCommon):
        ...
        if mask is not None:
            ...
            if len(mask.shape) == len(x.shape):
                dataset = tf.data.Dataset.from_tensor_slices(
                    (x.astype(CSMT_NUMPY_DTYPE), targets.astype(CSMT_NUMPY_DTYPE), mask.astype(CSMT_NUMPY_DTYPE),)
                ).batch(self.batch_size, drop_remainder=False)

            else:
                dataset = tf.data.Dataset.from_tensor_slices(
                    (
                        x.astype(CSMT_NUMPY_DTYPE),
                        targets.astype(CSMT_NUMPY_DTYPE),
                        np.array([mask.astype(CSMT_NUMPY_DTYPE)] * x.shape[0]),
                    )
                ).batch(self.batch_size, drop_remainder=False)

        else:
            dataset = tf.data.Dataset.from_tensor_slices(
                (x.astype(CSMT_NUMPY_DTYPE), targets.astype(CSMT_NUMPY_DTYPE),)
            ).batch(self.batch_size, drop_remainder=False)
    ...
    def _compute_perturbation(self, x: "tf.Tensor", y: "tf.Tensor", mask: Optional["tf.Tensor"]) -> "tf.Tensor":
        ...
        grad: tf.Tensor = self.estimator.loss_gradient(x, y) * tf.constant(
            1 - 2 * int(self.targeted), dtype=CSMT_NUMPY_DTYPE
        )
    ...
    def _apply_perturbation(
        ...
        x = x + tf.constant(eps_step, dtype=CSMT_NUMPY_DTYPE) * perturbation
