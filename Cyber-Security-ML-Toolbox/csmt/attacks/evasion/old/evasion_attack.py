...
from csmt.attacks.evasion.gradient_attack import BIMAttack, GradientEvasionAttack,FGSMAttack,PGDAttack,CWAttack,JSMAAttack,DeepFoolAttack,UniversalAttack
from csmt.attacks.evasion.old.de import DEEvasionAttack
...
from csmt.attacks.evasion.gradient_free.zoo import ZooAttack
...
def evasion_dict(model,algorithm,upper,lower,feature_importance,mask=None):
    ...
    if 'l1' in algorithm:
        ...
        return evasion_dic[algorithm](estimator=model,eps=0.2,eps_step=0.2,max_iter=1,norm=np.inf,upper=upper,lower=lower,feature_importance=feature_importance,mask=mask)
...
def EvasionAttack(attack_models,attack_model_name,trained_models,evasion_algorithm,X_test,y_test,upper=1,lower=0,feature_importance=None,mask=None):
    ...
    if len(attack_models)!=1 and len(evasion_algorithm)==1:
        ...
        # X_adv,y_adv,X_adv_path=TransferEnsembleEvasionAttack(attack_models,transfer_weight,attack_model_name,evasion_algorithm,X_test,y_test,upper=1,lower=0,feature_importance=None,mask=None)
...
def TransferEnsembleEvasionAttack(attack_models,transfer_weight,attack_model_name,evasion_algorithm,X_test,y_test,upper=1,lower=0,feature_importance=None,mask=None):
    # print("开始迁移集成攻击")
...
def EnsembleEvasionAttack(attack_models,evasion_algorithm,X_test,y_test,upper=1,lower=0,feature_importance=None,mask=None):
    ...
    evasion_weight=1.0/len(evasion_algorithm_arr)*np.ones(len(evasion_algorithm_arr),dtype=CSMT_NUMPY_DTYPE)
