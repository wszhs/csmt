...
class ZooAttack(EvasionAttack):
    ...
    def __init__(
        ...
        if self.use_resize:
            ...
            self._current_noise = np.zeros(dims, dtype=CSMT_NUMPY_DTYPE)
        else:
            self._current_noise = np.zeros((batch_size,) + self.estimator.input_shape, dtype=CSMT_NUMPY_DTYPE)
        self._sample_prob = np.ones(self._current_noise.size, dtype=CSMT_NUMPY_DTYPE) / self._current_noise.size
        ...
        if reset:
            ...
            if dims == x.shape:
                ...
                if x.shape == self._current_noise.shape:
                    ...
                    self._current_noise = np.zeros(x.shape, dtype=CSMT_NUMPY_DTYPE)
            else:
                resized_x = zoom(x, (1, dims[1] / x.shape[1], dims[2] / x.shape[2], dims[3] / x.shape[3],),)
                self._current_noise = np.zeros(dims, dtype=CSMT_NUMPY_DTYPE)
            self._sample_prob = np.ones(nb_vars, dtype=CSMT_NUMPY_DTYPE) / nb_vars
        else:
