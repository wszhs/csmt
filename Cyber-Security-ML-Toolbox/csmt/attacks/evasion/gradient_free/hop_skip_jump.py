...
from csmt.utils import to_categorical, check_and_transform_label_format
...
class HopSkipJump(EvasionAttack):
    ...
    def generate(self, x: np.ndarray, y: Optional[np.ndarray] = None, **kwargs) -> np.ndarray:
        ...
        if mask is not None:
            if len(mask.shape) == len(x.shape):
                mask = mask.astype(CSMT_NUMPY_DTYPE)
            else:
                mask = np.array([mask.astype(CSMT_NUMPY_DTYPE)] * x.shape[0])
        else:
    ...
    def _init_sample(
        ...
        if self.targeted:
            ...
            if adv_init is not None and init_pred == y:
                return adv_init.astype(CSMT_NUMPY_DTYPE), init_pred
            ...
            if adv_init is not None and init_pred != y_p:
                return adv_init.astype(CSMT_NUMPY_DTYPE), y_p
    ...
    def _compute_update(
        ...
        if self.norm == 2:
            rnd_noise = np.random.randn(*rnd_noise_shape).astype(CSMT_NUMPY_DTYPE)
        else:
            rnd_noise = np.random.uniform(low=-1, high=1, size=rnd_noise_shape).astype(CSMT_NUMPY_DTYPE)
        ...
        f_val = f_val.astype(CSMT_NUMPY_DTYPE)
