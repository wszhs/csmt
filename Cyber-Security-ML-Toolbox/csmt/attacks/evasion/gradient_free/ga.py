...
class GAEvasionAttack():

    @staticmethod
    def get_score(p):
        p=p.astype(CSMT_NUMPY_DTYPE)
        GAEvasionAttack.count=GAEvasionAttack.count+1
        score=GAEvasionAttack.estimator.predict(p.reshape(1,-1))
        return -score[0][0]
    ...
    def __init__(
        ...
        | paper link: https://dl.acm.org/doi/pdf/10.1145/3321707.3321749
        """

        GAEvasionAttack.estimator=estimator
        self.norm=norm
        self.eps=eps
    ...
    def _get_single_x(self,x,chrom_length):
        ...
        de = GA(func=GAEvasionAttack.get_score, n_dim=chrom_length, size_pop=10, max_iter=50, lb=lb, ub=ub)
